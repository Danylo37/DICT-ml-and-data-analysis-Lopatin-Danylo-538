import matplotlib.pyplot as plt
import sklearn.datasets
import numpy as np
import sklearn


def save_original_plot(X, Y, dataset_name):
    plt.figure(figsize=(8, 6))
    plt.scatter(X[0, :], X[1, :], c=Y, s=40, cmap=plt.get_cmap("Spectral"))
    plt.title(f"{dataset_name} - Original")
    plt.tight_layout()
    plt.savefig(f"results/task_2_{dataset_name}_original.png")
    plt.show()


def save_prediction_plot(model, X, Y, dataset_name, acc):
    plt.figure(figsize=(8, 6))
    plot_decision_boundary(model.get_decision_boundary_function(), X, Y)
    plt.title(f"{dataset_name} - ANN Prediction ({acc:.2f}%)")
    plt.tight_layout()
    plt.savefig(f"results/task_2_{dataset_name}_pred.png")
    plt.show()


def plot_decision_boundary(model, X, y):
    x_min, x_max = X[0, :].min() - 1, X[0, :].max() + 1
    y_min, y_max = X[1, :].min() - 1, X[1, :].max() + 1
    h = 0.01
    xx, yy = np.meshgrid(np.arange(x_min, x_max, h), np.arange(y_min, y_max, h))
    Z = model(np.c_[xx.ravel(), yy.ravel()])
    Z = Z.reshape(xx.shape)
    plt.contourf(xx, yy, Z, cmap=plt.get_cmap("Spectral"))
    plt.ylabel("x2")
    plt.xlabel("x1")
    plt.scatter(X[0, :], X[1, :], c=y, cmap=plt.get_cmap("Spectral"))


def sigmoid(x):
    return 1 / (1 + np.exp(-x))


def load_planar_dataset():
    np.random.seed(42)
    m = 400
    N = int(m / 2)
    D = 2
    X = np.zeros((m, D))
    Y = np.zeros((m, 1), dtype="uint8")
    a = 4

    for j in range(2):
        ix = range(N * j, N * (j + 1))
        t = np.linspace(j * 3.12, (j + 1) * 3.12, N) + np.random.randn(N) * 0.2
        r = a * np.sin(4 * t) + np.random.randn(N) * 0.2
        X[ix] = np.c_[r * np.sin(t), r * np.cos(t)]
        Y[ix] = j

    X = X.T
    Y = Y.T

    return X, Y


def load_extra_datasets():
    N = 200
    noisy_circles = sklearn.datasets.make_circles(n_samples=N, factor=0.5, noise=0.3)
    noisy_moons = sklearn.datasets.make_moons(n_samples=N, noise=0.2)
    blobs = sklearn.datasets.make_blobs(
        n_samples=N, random_state=42, n_features=2, centers=6
    )
    gaussian_quantiles = sklearn.datasets.make_gaussian_quantiles(
        mean=None,
        cov=0.5,
        n_samples=N,
        n_features=2,
        n_classes=2,
        shuffle=True,
        random_state=None,
    )
    no_structure = np.random.rand(N, 2), np.random.rand(N, 2)

    return noisy_circles, noisy_moons, blobs, gaussian_quantiles, no_structure
